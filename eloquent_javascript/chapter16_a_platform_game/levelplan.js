...
DomDisplay.prototype.scrollPlayerIntoView = function(state) {
    ...
    const left = this.dom.scrollLeft, right = left + right;
    const top = this.dom.scrollTop, bottom = top + height;

    const player = state.player;
    const center = player.pos.plus(player.size.times(.5)).times(scale)
...
Level.prototype.touches = function(pos, size, type) {
    ...
    for(const y = yStart; y < yEnd; y++) {
        for(const x = xStart; x < xEnd; x++) {
